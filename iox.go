...
//IoReadSeeker及为io.ReadSeeker
type IoReadSeeker struct {
	readSeeker io.ReadSeeker
}

//从io.readSeeker初始化
func New(rs io.ReadSeeker) *IoReadSeeker {
	f := new(IoReadSeeker)
	f.readSeeker = rs
	return f
}
...
func NewFromFile(fileName string) (*IoReadSeeker, error) {
...
//从切片初始化
func NewFromBytes(b []byte) *IoReadSeeker {
	return New(bytes.NewReader(b))
}

//关闭f
func (f *IoReadSeeker) Close() {
...
//从当前位置移动多少个字节
func (f *IoReadSeeker) Move(n int64) error {
...
//移动到某个位置
func (f *IoReadSeeker) MoveTo(pos int64) error {
...
//获得当前位置
func (f *IoReadSeeker) CurPos() (int64, error) {
	return f.readSeeker.Seek(0, io.SeekCurrent)
}

//获得长度
func (f *IoReadSeeker) Len() int64 {
	//防止文件位置发生移动
...
//读取n个字节,并移动指针
func (f *IoReadSeeker) ReadByte(n int) ([]byte, error) {
...
func (f *IoReadSeeker) ReadByteUint8() ([]byte, error) {
	n, err := f.ReadUint8()
	if err != nil {
		return nil, err
	}
...
func (f *IoReadSeeker) ReadByteUint16() ([]byte, error) {
	n, err := f.ReadUint16()
	if err != nil {
		return nil, err
	}
...
//读取若干字节,并移动指针,其中前2个字节表示后续待读取切片长度
func (f *IoReadSeeker) ReadByteUint16BigEndian() ([]byte, error) {
...
func (f *IoReadSeeker) ReadByteUint32() ([]byte, error) {
	n, err := f.ReadUint32()
	if err != nil {
		return nil, err
	}
...
//读取若干字节,并移动指针,其中前4个字节表示后续待读取切片长度
func (f *IoReadSeeker) ReadByteUint32BigEndian() ([]byte, error) {
...
func (f *IoReadSeeker) ReadByteUint64() ([]byte, error) {
	n, err := f.ReadUint64()
	if err != nil {
		return nil, err
	}
...
//读取若干字节,并移动指针,其中前8个字节表示后续待读取切片长度
func (f *IoReadSeeker) ReadByteUint64BigEndian() ([]byte, error) {
...
func (f *IoReadSeeker) ReadStringUint8() (string, error) {
	n, err := f.ReadUint8()
	if err != nil {
		return "", err
	}
...
func (f *IoReadSeeker) ReadStringUint16() (string, error) {
	n, err := f.ReadUint16()
	if err != nil {
		return "", err
	}
...
//读取若干字节,转化为文本,并移动指针,其中前2个字节表示后续待读取切片长度
func (f *IoReadSeeker) ReadStringUint16BigEndian() (string, error) {
	n, err := f.ReadUint16BigEndian()
	if err != nil {
		return "", err
	}
...
func (f *IoReadSeeker) ReadStringUint32() (string, error) {
	n, err := f.ReadUint32()
	if err != nil {
		return "", err
	}
...
//读取若干字节,转化为文本,并移动指针,其中前4个字节表示后续待读取切片长度
func (f *IoReadSeeker) ReadStringUint32BigEndian() (string, error) {
	n, err := f.ReadUint32BigEndian()
	if err != nil {
		return "", err
	}
...
func (f *IoReadSeeker) ReadStringUint64() (string, error) {
	n, err := f.ReadUint64()
	if err != nil {
		return "", err
	}
...
//读取若干字节,转化为文本,并移动指针,其中前8个字节表示后续待读取切片长度
func (f *IoReadSeeker) ReadStringUint64BigEndian() (string, error) {
	n, err := f.ReadUint64BigEndian()
	if err != nil {
		return "", err
	}
...
func (f *IoReadSeeker) ReadHexToString(n int) (string, error) {
	bt, err := f.ReadByte(n)
	if err != nil {
		return "", err
	}
...
func (f *IoReadSeeker) ReadString(n int) (string, error) {
	bt, err := f.ReadByte(n)
	if err != nil {
		return "", err
	}
...
func (f *IoReadSeeker) ReadStringTrimSpace(n int) (string, error) {
	bt, err := f.ReadByte(n)
	if err != nil {
		return "", err
	}
...
//读取一个uint8数,并移动指针
func (f *IoReadSeeker) ReadUint8() (uint8, error) {
	bt := make([]byte, 1)
...
//读取一个uint16数,并移动指针
func (f *IoReadSeeker) ReadUint16() (uint16, error) {
	bt := make([]byte, 2)
	_, err := f.readSeeker.Read(bt)
	if err != nil {
		return 0, err
...
//读取一个uint16数,并移动指针
func (f *IoReadSeeker) ReadUint16BigEndian() (uint16, error) {
	bt := make([]byte, 2)
	_, err := f.readSeeker.Read(bt)
	if err != nil {
		return 0, err
...
//读取一个uint32数,并移动指针
func (f *IoReadSeeker) ReadUint32() (uint32, error) {
	bt := make([]byte, 4)
	_, err := f.readSeeker.Read(bt)
	if err != nil {
		return 0, err
...
//读取一个uint32数,并移动指针
func (f *IoReadSeeker) ReadUint32BigEndian() (uint32, error) {
	bt := make([]byte, 4)
	_, err := f.readSeeker.Read(bt)
	if err != nil {
		return 0, err
...
//读取一个uint64数,并移动指针
func (f *IoReadSeeker) ReadUint64() (uint64, error) {
	bt := make([]byte, 8)
	_, err := f.readSeeker.Read(bt)
	if err != nil {
		return 0, err
...
//读取一个uint64数,并移动指针
func (f *IoReadSeeker) ReadUint64BigEndian() (uint64, error) {
	bt := make([]byte, 8)
	_, err := f.readSeeker.Read(bt)
	if err != nil {
		return 0, err
...
//判断f是否包含子串sep
func (f *IoReadSeeker) Contains(sep []byte) bool {
	return f.Index(sep) != -1
}

//计算f中共有多少个不重重叠的sep子串
func (f *IoReadSeeker) Count(sep []byte) int64 {
	return f.CountGen(0, f.Len()-1, sep)
}

//判断f中某段数据内有多少个不重叠的sep子串
func (f *IoReadSeeker) CountGen(beginPos, endPos int64, sep []byte) int64 {
	//防止文件位置发生移动
	initialPos, _ := f.CurPos()
	defer f.MoveTo(initialPos)
...
func (f *IoReadSeeker) Index(sep []byte) int64 {
	endPos := f.Len() - 1
...
func (f *IoReadSeeker) IndexGen(beginPos, endPos int64, sep []byte) int64 {
	//防止文件位置发生移动
	initialPos, _ := f.CurPos()
	defer f.MoveTo(initialPos)
...
func (f *IoReadSeeker) IndexN(beginPos int64, sep []byte, n int) int64 {
	//防止文件位置发生移动
	initialPos, _ := f.CurPos()
	defer f.MoveTo(initialPos)
	f.MoveTo(beginPos)
...
func (f *IoReadSeeker) LastIndex(sep []byte) int64 {
	endPos := f.Len() - 1
	return f.LastIndexGen(0, endPos, sep)
}

//查找数据,在的beginPos到endPos内查找sep子串,返回最后1次出现的位置,注意待搜索数据包含beginPos及endPos本身
func (f *IoReadSeeker) LastIndexGen(beginPos, endPos int64, sep []byte) int64 {
	//防止文件位置发生移动
	initialPos, _ := f.CurPos()
	defer f.MoveTo(initialPos)
	//先对输入值合法性进行判断,不合法则直接退出,强制用户检察
...
//从当前位置向前读取n个字节,并移动指针
func (f *IoReadSeeker) ReadByteReverse(n int) ([]byte, error) {
	//先向前移动n个位置
	err := f.Move(int64(-n))
	if err != nil {
		return nil, err
